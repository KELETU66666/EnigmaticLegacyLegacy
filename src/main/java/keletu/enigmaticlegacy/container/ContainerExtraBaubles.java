...
public class ContainerExtraBaubles extends Container {
    /**
    ...
    public IExtendedBaublesItemHandler extendedBaubles;
    /**
    ...
    public ContainerExtraBaubles(InventoryPlayer playerInv, boolean par2, EntityPlayer player) {
        ...
        extendedBaubles = player.getCapability(EnigmaticCapabilities.CAPABILITY_BAUBLES, null);
        ...
        for (int k = 0; k < 4; k++) {
            ...
            this.addSlotToContainer(new Slot(playerInv, 36 + (3 - k), 6, 26 + k * 18) {
                @Override
        ...
        this.addSlotToContainer(new SlotExtraBauble(player, extendedBaubles, 0, 42, 8));
        this.addSlotToContainer(new SlotExtraBauble(player, extendedBaubles, 1, 78, 26));
        this.addSlotToContainer(new SlotExtraBauble(player, extendedBaubles, 2, 6, 98));
        this.addSlotToContainer(new SlotExtraBauble(player, extendedBaubles, 3, 78, 62));


        this.addSlotToContainer(new SlotBauble(player, baubles, 0, 24, 8));
        this.addSlotToContainer(new SlotBauble(player, baubles, 1, 24, 98));
        this.addSlotToContainer(new SlotBauble(player, baubles, 2, 42, 98));
        this.addSlotToContainer(new SlotBauble(player, baubles, 3, 78, 80));
        this.addSlotToContainer(new SlotBauble(player, baubles, 4, 60, 8));
        this.addSlotToContainer(new SlotBauble(player, baubles, 5, 78, 44));
        this.addSlotToContainer(new SlotBauble(player, baubles, 6, 60, 98));


        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 9; ++j) {
                this.addSlotToContainer(new Slot(playerInv, j + (i + 1) * 9, 6 + j * 18 + (j > 4 ? 10 : 0), 119 + i * 18));
            }
        }

        for (int i = 0; i < 9; ++i) {
            this.addSlotToContainer(new Slot(playerInv, i, 6 + i * 18 + (i > 4 ? 10 : 0), 173));
        }

        this.addSlotToContainer(new Slot(playerInv, 40, 78, 98) {
            @Override
            public boolean isItemValid(ItemStack stack) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            // inv -> bauble
                ...
                for (int baubleSlot : bauble.getBaubleType(itemstack).getValidSlots()) {
                    if (bauble.canEquip(itemstack1, thePlayer) && !this.inventorySlots.get(baubleSlot + 9).getHasStack() && !this.mergeItemStack(itemstack1, baubleSlot + 9, baubleSlot + 10, false)) {
                        return ItemStack.EMPTY;
                    }
            ...
            //TODO : Check
            // inv -> extended bauble
            else if (itemstack.hasCapability(EnigmaticCapabilities.CAPABILITY_ITEM_BAUBLE, null)) {
                IExtendedBauble bauble = itemstack.getCapability(EnigmaticCapabilities.CAPABILITY_ITEM_BAUBLE, null);
                for (int baubleSlot : bauble.getBaubleType(itemstack).getValidSlots()) {
            ...
            if (itemstack1.isEmpty() && !baubles.isEventBlocked() && slot instanceof SlotExtraBauble &&
                    itemstack.hasCapability(EnigmaticCapabilities.CAPABILITY_ITEM_BAUBLE, null)) {
                itemstack.getCapability(EnigmaticCapabilities.CAPABILITY_ITEM_BAUBLE, null).onUnequipped(itemstack, playerIn);
            }
